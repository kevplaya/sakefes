  ...
  return (
    ...
    <div className="relative w-full aspect-square" style={{ touchAction: "manipulation" }}>
      ...
      >
        ...
        <text x={xScale(CENTER) + 6} y={yScale(CENTER) - 6} fontSize="11" fill="#b6313a" fontFamily="serif">中 (6,6)</text>
        ...
        {hover && (
          <g pointerEvents="none">
            ...
            <text
              ...
              y={Math.max(8, yScale(hover.aroma) - 56) + 19}
              fontSize="13"
              fill="#f4ecdb"
              fontFamily="serif"
            >
              {hover.product.length > 28 ? hover.product.slice(0, 28) + "…" : hover.product}
            </text>
            <text
              x={Math.min(W - 260, Math.max(8, xScale(hover.richness) + 10)) + 10}
              y={Math.max(8, yScale(hover.aroma) - 56) + 38}
              fontSize="12"
              fill="#d9c89a"
              fontFamily="serif"
            >
...
function QLabel({ x, y, title, sub, ko, color }: { x: number; y: number; title: string; sub: string; ko: string; color: string }) {
  return (
    <g pointerEvents="none">
      <text x={x} y={y - 10} textAnchor="middle" fontSize="44" fill={color} opacity="0.16" fontFamily="serif" fontWeight={700}>{title}</text>
      <text x={x} y={y + 18} textAnchor="middle" fontSize="12" fill={color} opacity="0.7" fontFamily="serif" letterSpacing="0.2em">{sub} · {ko}</text>
    </g>
  );
}
